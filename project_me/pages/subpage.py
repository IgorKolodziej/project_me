...
layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            dl.Map(
                [dl.TileLayer(), dl.Polyline(id='route', positions=route1_coords)],
                id='map',
        ...
        ], className='left-column', style={'padding-right':'20px'},),

        # Right Column for RadioItems and Labels
        html.Div([
            html.Div([
    dmc.Text("Każdy z nas wybrał się na indywidualny bieg w okolicach Woli, by w sportowym duchu porównać czasy i trasy naszych przebieżek.", color="black", size="h3", align="left"),
    dmc.Text("Mateusz, Igor oraz Nazari, wyposażeni w smartfony, wyruszyli w różne strony, eksplorując malownicze ścieżki i ulice dzielnicy.", color="black", size="h3", align="left",style={'marginTop': '20px'}),
]),
            ...
            dcc.Slider(
                ...
                updatemode='drag',
                #marks=None
                 marks=format_times(route1_times)  # Use the updated function
            )
        ], className='right-column',style={'display': 'grid','gridTemplateRows': '1fr 1fr','gap':'10px'})
    ], style={
        'display': 'grid',
